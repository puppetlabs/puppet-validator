...
1. drag this bookmarklet link to your bookmarks menu.
    * [javascript:var win=window.open('', 'linter', '');var form=win.document.createElement('form');form.action='http://puppetlinter.com/validate';form.method='POST';var code=win.document.createElement('textarea');code.name='code';code.value=window.getSelection().toString();form.appendChild(code);win.document.body.appendChild(form);form.submit();](Validate Puppet Code)
